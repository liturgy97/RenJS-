...
class AudioController {
  constructor(debug) {
    ...
    console.log("%c 🔊 Audio module active", "color: #FFC300");
  }
  ...
  setVolume(audio, volume = settings.sfxVolume) {
    volume = valBounds(volume, 0, 100);
    let volumePercent = volume / 100;
    let sample = this._globalAudio[audio];
    sample.forEach(function (s) {
      s.volume = volumePercent;
      s.setAttribute("volume", volumePercent);
    });
  }
  ...
  getAudioArrayNames() {
    return this._audioArray.map((obj) => obj.name);
  }

  setSFXVolume(volume = settings.sfxVolume) {
    volume = valBounds(volume, 0, 100);
    settings.sfxVolume = volume;
    let array = arrayDifference(
      this.getAudioArrayNames(),
      settings.musicTracks
    );
    array.forEach((trackName) => {
      this.setVolume(trackName, settings.sfxVolume);
    });
  }
  setMusicVolume(volume = settings.musicVolume) {
    volume = valBounds(volume, 0, 100);
    settings.musicVolume = volume;
    settings.musicTracks.forEach((trackName) => {
      this.setVolume(trackName, settings.musicVolume);
    });
    let all = document.querySelector("audio");
    if (all) all.volume = settings.musicVolume / 100;
  }
  ...
  globalUnmute() {
    ...
    let all = document.querySelector("audio");
    if (all) all.volume = this.musicVolume;
  }
  ...
  createAnalyser(dialogueElem, bgAudioElem) {
    if (!this.analyser) {
      ...
      this.dialogueSource = this.audioCtx.createMediaElementSource(
        this.dialogue
      );
  ...
  drawWaveform() {
    ...
    setInterval(function () {
      ...
      for (let i = 0; i < 4; i++) {
        document
          .querySelectorAll(".equaliser_part")
          [i].getElementsByClassName("equaliser_part__inner")[0].style.height =
          that.frequencyData[i] / 18 + "px";
      }
    }, 10);
  }
  ...
  postProcess(src) {
    ...
    if (!this.audioCtx) {
      ...
      document.getElementsByClassName(
        "_audio"
      )[0].innerHTML = `<audio crossorigin='anonymous'><source src='${sample.src}' type='audio/mp3'></audio>`;
